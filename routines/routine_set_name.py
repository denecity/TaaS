...
"""
	...
	async def perform(self, session: Turtle._Session, config: Any | None) -> None:
		...
		ok = False
		try:
			res = await session.eval(f'((function() return set_name_tag("{escaped}") end)())')
			# session.eval returns the value; ensure boolean
			ok = bool(res)
		except Exception as e:
			logger.error("Turtle %d: set_name_tag eval failed: %s", session._turtle.id, e)
			return
		if ok:
